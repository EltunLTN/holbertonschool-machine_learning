...
class WGAN_clip(keras.Model):
    """
    WGAN_clip Class extending keras.Model
    """
    ...
    def __init__(
        ...
        # discriminator_loss = E[D(G(z))] - E[D(x_real)]
    ...
    def get_fake_sample(self, size=None, training=False):
        """
        Generates a fake sample using the generator network
        """
        if not size:
    ...
    def get_real_sample(self, size=None):
        """
        Gets a random batch sample of real examples
        """
        if not size:
            size = self.batch_size
    ...
    def train_step(self, useless_argument):
        """
        Performs one training step for WGAN_clip
        """
        # Train Discriminator (Critic) for disc_iter iterations
